...
const AdminDoctors = () => {
  ...
  const updateApprovalStatus = async (id, newStatus) => {
    try {
      ...
      console.log(`🔄 Updating doctor ${id} status to ${newStatus}`);
      ...
      if (response.ok) {
        console.log(`✅ Doctor ${newStatus} successfully`);
        // Refresh the page to show updated data
        window.location.reload();
      } else {
        console.error("❌ Failed to update doctor status:", response.status, data.message || data);
      }
    } catch (error) {
